...
function metronome() {
    return {
        ...
        get tickInterval() {
            return this.timingService
                ? this.timingService.calculateTickInterval(
                      this.bpm,
                      this.subdivisions
                  )
                : 500;
        ...
        get beatsPerMeasure() {
            return this.timingService
                ? this.timingService.parseTimeSignature(this.timeSignature)
                      .beats
                : 4;
        },
        ...
        tick() {
            ...
            if (this.showSubdivisions && this.mode !== "remote") {
                this.currentSubdivision =
                    this.timingService.getCurrentSubdivision(
                        this.subdivisionCount,
                        this.subdivisions
                    );
            }
        ...
        tapTempo() {
            ...
            if (this.lastTapTime) {
                ...
                if (this.tapTimes.length >= 2) {
                    const newBpm = this.timingService.calculateTapTempoBpm(
                        this.tapTimes
                    );
                    if (newBpm) {
                        this.setBpm(newBpm);
